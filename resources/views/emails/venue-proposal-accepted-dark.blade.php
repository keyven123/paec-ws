@php
    $data = $data ?? [];
    $emailPageTitle = $data['email_page_title'] ?? 'Proposal Accepted';
    $emailHeadline = $data['email_headline'] ?? 'Great news — your proposal was accepted';
...
<head>
    ...
    <style type="text/css">
        ...
        .accepted-card { padding:20px 22px; background:linear-gradient(135deg,#14532d 0%,#052e16 100%); border:1px solid #16a34a; }
        .accepted-label { color:#bbf7d0; font-size:12px; font-weight:700; letter-spacing:.08em; text-transform:uppercase; margin:0 0 8px; }
        .accepted-amount { color:#fff; font-size:22px; font-weight:700; margin:0; }
...
<div class="container">
    ...
    <div class="content">
        ...
        <p style="margin:0 0 18px;">
            <span class="status-badge">{{ $inquiry['status'] ?? 'Accepted' }}</span>
        ...
        @if(!empty($proposal['amount_label']))
            <div class="card accepted-card">
                <p class="accepted-label">Accepted proposal amount</p>
                <p class="accepted-amount">{{ $proposal['amount_label'] }}</p>
            </div>
        @endif
        ...
        <div class="tip-box">
            ...
            Send a deposit request from the inquiry workflow to secure the booking date, and continue coordinating with the customer in chat.
        </div>
    </div>

    <div class="footer">
        <p>{!! $emailFooterThanks !!}<br>This is an automated notice about an accepted venue proposal on Ticketoc.</p>
        <p>
