@php
    $data = $data ?? [];
    $emailPageTitle = $data['email_page_title'] ?? 'Proposal Declined';
    $emailHeadline = $data['email_headline'] ?? 'A customer declined your proposal';
...
<head>
    ...
    <style type="text/css">
        ...
        .status-badge { display:inline-block; background:#b91c1c; color:#fff; border-radius:999px; padding:4px 12px; font-size:11px; font-weight:700; letter-spacing:.06em; text-transform:uppercase; }
        .card { background:#1a1a1a; border:1px solid #2a2a2a; border-radius:12px; margin-bottom:18px; overflow:hidden; }
        .declined-card { padding:20px 22px; background:linear-gradient(135deg,#7f1d1d 0%,#450a0a 100%); border:1px solid #b91c1c; }
        .declined-label { color:#fecaca; font-size:12px; font-weight:700; letter-spacing:.08em; text-transform:uppercase; margin:0 0 8px; }
        .declined-amount { color:#fff; font-size:22px; font-weight:700; margin:0; }
        ...
        .tip-box { margin-top:18px; padding:14px 16px; background:#450a0a; border:1px solid #b91c1c; border-radius:8px; color:#fecaca; font-size:13px; }
...
<div class="container">
    ...
    <div class="content">
        ...
        <p style="margin:0 0 18px;">
            <span class="status-badge">{{ $inquiry['status'] ?? 'In Discussion' }}</span>
            @if(!empty($inquiry['reference']))
                <span style="margin-left:10px;color:#a1a1aa;font-size:13px;">Ref #{{ $inquiry['reference'] }}</span>
            @endif
        </p>

        @if(!empty($proposal['amount_label']))
            <div class="card declined-card">
                <p class="declined-label">Declined proposal amount</p>
                <p class="declined-amount">{{ $proposal['amount_label'] }}</p>
            </div>
        @endif
        ...
        <p style="margin:22px 0 18px;">
            <a href="{{ $manageUrl }}" target="_blank" rel="noopener" style="display:inline-block;padding:12px 18px;border-radius:8px;text-decoration:none;font-weight:600;background:#b91c1c;color:#ffffff !important;">
                Open inquiry chat
            </a>
        </p>

        <div class="tip-box">
            <strong style="color:#fecaca;">What to do next</strong><br>
            Reply in chat, adjust your proposal, and send a revised document when ready. The customer can continue the discussion with you inside Ticketoc.
        </div>
    </div>

    <div class="footer">
        <p>{!! $emailFooterThanks !!}<br>This is an automated notice about a declined venue proposal on Ticketoc.</p>
        <p>
