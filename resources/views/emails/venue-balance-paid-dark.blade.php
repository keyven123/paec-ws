@php
    $data = $data ?? [];
    $emailPageTitle = $data['email_page_title'] ?? 'Balance Paid';
    $emailHeadline = $data['email_headline'] ?? 'Final balance received — booking fully confirmed';
...
<head>
    ...
    <style type="text/css">
        ...
        .status-badge { display:inline-block; background:#047857; color:#fff; border-radius:999px; padding:4px 12px; font-size:11px; font-weight:700; letter-spacing:.06em; text-transform:uppercase; }
        .card { background:#1a1a1a; border:1px solid #2a2a2a; border-radius:12px; margin-bottom:18px; overflow:hidden; }
        .payment-card { padding:20px 22px; background:linear-gradient(135deg,#064e3b 0%,#022c22 100%); border:1px solid #10b981; }
        .payment-label { color:#a7f3d0; font-size:12px; font-weight:700; letter-spacing:.08em; text-transform:uppercase; margin:0 0 8px; }
        .payment-amount { color:#fff; font-size:22px; font-weight:700; margin:0; }
        .payment-meta { color:#6ee7b7; font-size:13px; margin:8px 0 0; }
        ...
        .tip-box { margin-top:18px; padding:14px 16px; background:#022c22; border:1px solid #10b981; border-radius:8px; color:#a7f3d0; font-size:13px; }
...
<div class="container">
    ...
    <div class="content">
        ...
        <p style="margin:0 0 18px;">
            <span class="status-badge">{{ $inquiry['status'] ?? 'Fully Paid' }}</span>
            @if(!empty($inquiry['reference']))
                <span style="margin-left:10px;color:#a1a1aa;font-size:13px;">Ref #{{ $inquiry['reference'] }}</span>
            @endif
        </p>

        @if(!empty($payment['balance_label']))
            <div class="card payment-card">
                <p class="payment-label">Final balance received</p>
                <p class="payment-amount">{{ $payment['balance_label'] }}</p>
                @if(!empty($payment['additional_charges_label']))
                    <p class="payment-meta">Includes additional charges of {{ $payment['additional_charges_label'] }}</p>
                @endif
                @if(!empty($payment['deposit_label']))
                    <p class="payment-meta">Deposit previously paid: {{ $payment['deposit_label'] }}</p>
                @endif
                @if(!empty($payment['paid_at']))
                    <p class="payment-meta">Paid on {{ $payment['paid_at'] }}</p>
                @endif
            </div>
        @endif
        ...
        <p style="margin:22px 0 18px;">
            <a href="{{ $manageUrl }}" target="_blank" rel="noopener" style="display:inline-block;padding:12px 18px;border-radius:8px;text-decoration:none;font-weight:600;background:#059669;color:#ffffff !important;">
                Open inquiry &amp; coordinate event
            </a>
        </p>

        <div class="tip-box">
            <strong style="color:#a7f3d0;">What to do next</strong><br>
            The booking is fully confirmed. Customer contact details are now available in the inquiry — reach out to finalize arrangements and mark the booking complete after the event.
        </div>
    </div>

    <div class="footer">
        <p>{!! $emailFooterThanks !!}<br>This is an automated notice about a venue balance payment on Ticketoc.</p>
        <p>
