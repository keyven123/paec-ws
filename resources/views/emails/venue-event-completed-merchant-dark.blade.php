@php
    $data = $data ?? [];
    $emailHeadline = $data['email_headline'] ?? 'Congratulations on another successful event!';
...
<head>
    ...
    <title>{{ $data['email_page_title'] ?? 'Event Completed' }}</title>
    <style type="text/css">
        ...
        .success-card { padding:20px 22px; background:linear-gradient(135deg,#064e3b 0%,#022c22 100%); border:1px solid #10b981; text-align:center; }
        .success-text { color:#a7f3d0; font-size:15px; margin:0; }
...
<div class="container">
    ...
    <div class="content">
        ...
        <p style="margin:0 0 18px;">
            <span class="status-badge">{{ $inquiry['status'] ?? 'Completed' }}</span>
        ...
        <div class="card success-card">
            <p class="success-text">🎊 Another event successfully hosted through Ticketoc!</p>
        </div>
        ...
        <div class="card">
            <table role="presentation" class="detail-grid" width="100%" cellpadding="0" cellspacing="0">
                ...
                @if(!empty($event['completed_at']))
                    <tr>
                        <td class="detail-label">Marked complete</td>
                        <td class="detail-value">{{ $event['completed_at'] }}</td>
                    </tr>
                @endif
            </table>
        </div>
        ...
        <p style="margin:22px 0 18px;">
            <a href="{{ $manageUrl }}" target="_blank" rel="noopener" style="display:inline-block;padding:12px 18px;border-radius:8px;text-decoration:none;font-weight:600;background:#059669;color:#ffffff !important;">
                View completed inquiry
            </a>
        </p>

        <div class="tip-box">
            <strong style="color:#a7f3d0;">Keep the momentum going</strong><br>
            A great event experience often leads to repeat bookings and referrals. Stay in touch with your customer and keep your venue calendar active on Ticketoc.
        </div>
    </div>

    <div class="footer">
        <p>{!! $emailFooterThanks !!}<br>This is an automated notice that a venue booking was marked complete on Ticketoc.</p>
        <p>
