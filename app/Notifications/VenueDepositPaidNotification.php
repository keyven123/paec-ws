...
class VenueDepositPaidNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $payload = [
            'email_page_title' => 'Deposit Received',
            'email_headline' => 'Deposit received — event date tentatively reserved',
            'email_intro' => ($inquiry->full_name ?? 'A customer')
                . ' has paid the deposit for '
                . ($venue?->name ?? 'your venue')
                . '. The event date is now tentatively reserved while you prepare final billing.',
            ...
            'deposit' => [
                'amount_label' => $depositLabel,
                'paid_at' => $inquiry->deposit_paid_at?->format('l, F d, Y g:i A'),
            ],
            'event' => [
        ...
        return (new MailMessage())
            ->view('emails.venue-deposit-paid-dark', ['data' => $payload])
            ->subject('[Ticketoc] Deposit received for ' . $venueName);
    }
